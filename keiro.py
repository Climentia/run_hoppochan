...
def keiro(origin_in, destination_in):
    # try:
    with open("./key/google_key.txt", "r", encoding="utf-8") as fr:
        MY_API_KEY = fr.readline()
    URL_FORMAT = "https://maps.googleapis.com/maps/api/directions/json?origin={}&destination={}&mode={}&key={}&avoid=tools"
    ...
    with open("./data/place_log.csv", "w", encoding="cp932") as fwlog:
...
def convert_json_to_csv():
    with open("./data/route_latlon.json", "r", encoding="utf-8", errors="ignore") as f:
        # sjson_open = open("./data/route_latlon.json", "r")
...
def get_data():
    ...
    json_load = json.load(json_open, encoding="utf-8")
...
def interpolation():
    with open("./data/route_point.csv", "r") as fr:
